...
#define Warning(s) Utils::ConsolePrint(_("warning: %s\n"), s)
#define ConMsg(s) Utils::ConsolePrint(_("msg: %s\n"), s);
...
CCommand::CCommand()
{
	if (!s_bBuiltBreakSet)
	{
		...
		CharacterSetBuild(&s_BreakSet, _("{}()':"));
	}
...
void ConVar::InternalSetValue(const char* value)
{
	...
	{
		snprintf(tempVal, sizeof(tempVal), _("%f"), fNewValue);
		val = tempVal;
	}
...
void ConVar::InternalSetFloatValue(float fNewValue)
{
	...
	{
		char tempVal[32];
		snprintf(tempVal, sizeof(tempVal), _("%f"), m_fValue);
...
void ConVar::InternalSetIntValue(int nValue)
{
	...
	{
		char tempVal[32];
		snprintf(tempVal, sizeof(tempVal), _("%d"), m_nValue);
		ChangeStringValue(tempVal, flOldValue);
	}
	else
	{
...
void ConVarRef::Init(const char* pName, bool bIgnoreMissing)
{
	...
	if (!IsValid())
	{
		...
		if (gm::SDK::Get().CVar() || bFirst)
		{
			if (!bIgnoreMissing)
			{
				Utils::ConsolePrint("ConVarRef %s doesn't point to an existing ConVar\n", pName);
			}
			bFirst = false;
		}
	}
