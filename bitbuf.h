...
BITBUF_INLINE void bf_write::WriteUBitLong(unsigned int curData, int numbits, bool bCheckRange)
{
	...
	int i = mask2 & 1;
	unsigned long dword1 = 0;//LoadLittleDWord(pOut, 0);
	unsigned long dword2 = 0;//LoadLittleDWord(pOut, i);
...
BITBUF_INLINE unsigned int bf_read::ReadUBitLong(int numbits)
{
	...
	unsigned int dw1 = 0;// LoadLittleDWord((unsigned long*)m_pData, iWordOffset1) >> iStartBit;
	unsigned int dw2 = 0;// LoadLittleDWord((unsigned long*)m_pData, iWordOffset2) << (32 - iStartBit);
